...
#' the derivative holdings that meet the keyword crtiera from the
...
#' each row represents a derivative holding The holding
...
#' @export
derivativeHoldingsPullAndScrape <- function(quarter, year, form, name, email, footnoteKeywords = NA, issuerKeywords = NA, issuerTradingSymbol = NA, rptOwnerKeywords = NA) {
  ...
  dat <- as.data.frame(matrix(, ncol=54, nrow=nrow(pull)*6))
